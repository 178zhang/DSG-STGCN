function [sources_int, sources_nonint, P_ar] = generate_sources_ar(fs, len, bandpass, ar_source_number)
...
N = fs*len;
P_ar = 5;
Nchanel = ar_source_number;
rep = 1;
while rep
  sources_nonint = zeros(N+100, Nchanel);
  for mm = 1: Nchanel
     sources_nonint(:, mm) = gen_ar_uni(N+100, P_ar);
  end
...
rep = 1;
while rep
  sources_int = gen_ar_anychannel(N+100, P_ar, ar_source_number)';
